...
const describeCard = (card) => {
  ...
  if (card.offense && !card.defense) {
    ...
    messages.push('This tactic is a development, which can be used instead of a defensive tactic.');
  }
  const extrasMap = {
    0: '0',
    1: 'some',
  ...
  if (card.tank) {
    messages.push('This tactic will take the damage for all allies in it\'s battle zone.');
  }
  if (card.shy) {
    messages.push('This tactic will run away to the other ship if an enemy tactic is played here.');
  }
