    ...
    if (card.planet) {
      ...
      planet = {
        ...
        [playerCards]: card.tank ?
          [addCardDefaults(cardId, card), ...planet[playerCards]] :
          [...planet[playerCards], addCardDefaults(cardId, card)],
      };
    } else if (card.space) {
      // eslint-disable-next-line no-param-reassign
      ships = {
        ...
        [ship]: {
          ...ships[ship],
          [playerCards]: card.tank ?
            [addCardDefaults(cardId, card), ...ships[ship][playerCards]] :
            [...ships[ship][playerCards], addCardDefaults(cardId, card)],
        },
      };
    }
  });
...
const defenseChoices = (state: State, myDeckKey: "aiDeck" | "playerDeck", myShipKey, opponentCardsKey) => {
  ...
  if (zonesToDefend.length === 1) {
    zoneDefense = pickRandomCards(state[myDeckKey], 2,
  ...
  const development = pickRandomCards(state[myDeckKey], 1,
