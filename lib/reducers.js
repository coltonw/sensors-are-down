...
function createDecks({ state, action }: SubReducerObj): SubReducerObj {
  let playerDeck = pickRandomCards(action.allCards, 5);
  while (!validDeck(playerDeck) || (action.includeCards &&
    ...
    playerDeck = pickRandomCards(action.allCards, 5);
  }
  let aiDeck = pickRandomCards(action.allCards, 5);
  while (!validDeck(aiDeck)) {
    aiDeck = pickRandomCards(action.allCards, 5);
  }
  return {
    state: ({
      ...state,
