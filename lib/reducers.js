...
function runCombat({ state, action }: SubReducerObj): SubReducerObj {
  ...
  const postCombatState = {
    ...
    ships: {
      ...state.ships,
      playerShip: combatHelper(state.ships.playerShip, specialStrength),
      aiShip: combatHelper(state.ships.aiShip, specialStrength),
    },
    planet: clearExtraEntrenchment(combatHelper(state.planet, specialStrength)),
  };
  return {
