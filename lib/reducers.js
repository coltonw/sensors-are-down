...
const { playSpecials, getSpecialStrength } = require('./specials');
...
function validDeck(deck: CardsObj): boolean {
  ...
  return shipDefenseCards.length > 0 && planetCards.length > 0;
}

function createDecks({ state, action }: SubReducerObj): SubReducerObj {
  let playerDeck = pickRandomCards(action.allCards, 4);
  while (!validDeck(playerDeck) || (action.includeCards &&
    ...
    playerDeck = pickRandomCards(action.allCards, 4);
  }
  let aiDeck = pickRandomCards(action.allCards, 4);
  while (!validDeck(aiDeck)) {
    aiDeck = pickRandomCards(action.allCards, 4);
  }
  return {
    state: ({
      ...state,
