...
function staccatoFliers(state) {
  const playerShipDamage = state.ships.playerShip.playerCards.reduce(
    ...
    _.filter(state.ships.playerShip.aiCards,
      card =>
        card.cardId === 'staccatoFliers' && card.strength >= 2).length > 0;
...
function treeOfTheStars(preCombatState, state) {
  ...
  if (playerNumDeadTrees > 0) {
    playerDeck = {
      ...playerDeck,
      starSplinters: {
        ...(playerDeck.starSplinters ?
          playerDeck.starSplinters :
          state.allCards.starSplinters),
        count: (playerNumDeadTrees * (state.allCards.starSplinters.count || 1)) +
          (playerDeck.starSplinters ?
            playerDeck.starSplinters.count || 1 :
            0),
      },
    };
  }
  ...
  if (aiNumDeadTrees > 0) {
    aiDeck = {
      ...aiDeck,
      starSplinters: {
        ...(aiDeck.starSplinters ?
          aiDeck.starSplinters :
          state.allCards.starSplinters),
        count: (aiNumDeadTrees * (state.allCards.starSplinters.count || 1)) +
          (aiDeck.starSplinters ?
            aiDeck.starSplinters.count || 1 :
            0),
      },
    };
  }
