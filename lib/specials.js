...
function planetaryBombers(state: State): State {
  const { playerCards, playerGraveyard } = state.planet.playerCards.reduce((acc, card) => {
    if (card.phasePlayed === 'defense' && card.turnPlayed === state.currentTurn) {
      return {
        playerCards: [...acc.playerCards, card],
        playerGraveyard: acc.playerGraveyard,
      };
    }
    return {
      playerCards: acc.playerCards,
      playerGraveyard: [...acc.playerGraveyard, card],
    };
  }, { playerCards: [], playerGraveyard: state.planet.playerGraveyard });
  const { aiCards, aiGraveyard } = state.planet.aiCards.reduce((acc, card) => {
    if (card.phasePlayed === 'defense' && card.turnPlayed === state.currentTurn) {
      return {
        aiCards: [...acc.aiCards, card],
        aiGraveyard: acc.aiGraveyard,
      };
    }
    return {
      aiCards: acc.aiCards,
      aiGraveyard: [...acc.aiGraveyard, card],
    };
  }, { aiCards: [], aiGraveyard: state.planet.aiGraveyard });
  return {
    ...state,
...
function barbedClusterShips(state) {
  const choicesKey = getCurrentChoicesKey(state);
  const choices = getCurrentChoices(state);
  if (choices && choicesKey) {
    const playerDeadClusters = _.filter(
      state.planet.playerGraveyard,
      (card: PlayedCard): boolean =>
        card.cardId === 'barbedClusterShips').length;
    const aiCards = _.mapValues(choices.aiCards,
      card =>
        (card.space ?
        {
          ...card,
          strength: card.strength - playerDeadClusters,
        } :
          card));
    const aiDeadClusters = _.filter(
      state.planet.aiGraveyard,
      (card: PlayedCard): boolean =>
        card.cardId === 'barbedClusterShips').length;
    const playerCards = _.mapValues(choices.playerCards,
      card =>
        ...
        {
          ...card,
          strength: card.strength - aiDeadClusters,
        } :
          card));
    return {
      ...state,
...
function stasisField(state) {
  ...
  const freeze = card =>
    ...
    card);
  if (choices) {
    let playerChoices = choices.playerCards;
    let aiChoices = choices.aiCards;
    let playerShipPlayerCards = state.ships.playerShip.playerCards;
    let playerShipAiCards = state.ships.aiShip.aiCards;
    let aiShipPlayerCards = state.ships.aiShip.playerCards;
    let aiShipAiCards = state.ships.aiShip.aiCards;
    let planetPlayerCards = state.planet.playerCards;
    let planetAiCards = state.planet.aiCards;
    if (choices && choices.playerCards.stasisField) {
      aiChoices = _.mapValues(aiChoices, freeze);
      playerShipAiCards = playerShipAiCards.map(freeze);
      aiShipAiCards = aiShipAiCards.map(freeze);
      planetAiCards = planetAiCards.map(freeze);
    }
    if (choices && choices.aiCards.stasisField) {
      playerChoices = _.mapValues(playerChoices, freeze);
      playerShipPlayerCards = playerShipPlayerCards.map(freeze);
      aiShipPlayerCards = aiShipPlayerCards.map(freeze);
      planetPlayerCards = planetPlayerCards.map(freeze);
    }
    const developedState: State = {
      ...state,
      [choiceKey]: {
        playerCards: playerChoices,
        aiCards: aiChoices,
      },
      ships: {
        ...state.ships,
      ...
      planet: {
        ...state.planet,
        playerCards: planetPlayerCards,
        aiCards: planetAiCards,
      },
    };
    return deleteDevelopment(developedState, 'stasisField');
  }
  return state;
}

function morphingPodWalkers(state) {
  const choiceKey = getCurrentChoicesKey(state);
  if (choiceKey && state[choiceKey]) {
    const morph = (card, cardId) => {
      if (cardId === 'morphingPodWalkers') {
        if (choiceKey === 'offenseCardChoices') {
          return {
            ...card,
            damageStrength: (card.damageStrength || 0) + 2,
          };
        }
        return {
          ...card,
          armorStrength: (card.armorStrength || 0) + 2,
        };
      }
      return card;
    };
    return {
      ...
      [choiceKey]: {
        ...state[choiceKey],
        playerCards: _.mapValues(state[choiceKey].playerCards,
          morph),
        aiCards: _.mapValues(state[choiceKey].aiCards,
          morph),
      },
    };
  }
