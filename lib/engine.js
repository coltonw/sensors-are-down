...
function selectCard({ state, action }) {
  if (state.defenseCardChoices) {
    ...
    console.log('narrowing down choices');
    console.log(action.cardId);
    console.log(_.pick(state.offenseCardChoices.playerCards, action.cardId));
    return {
      state: _.assign({}, state, {
  ...
  _.forEach(Object.keys(cardChoices[playerCards]), (cardId) => {
    ...
    if (card.planet) {
      ...
      planet = _.assign({}, planet, {
        [playerCards]: [...planet[playerCards], _.assign({}, card, {
          cardId,
        })],
      });
    } else if (card.ship) {
      ...
      ships = _.assign({}, ships, {
        [ship]: _.assign({}, ships[ship], {
          [playerCards]: [...ships.playerShip[playerCards], _.assign({}, card, {
            cardId,
          })],
        }),
      });
...
function playCards({ state, action }) {
  ...
  if (state.defenseCardChoices) {
    ...
    console.log('Offense card played:');
    console.log(state.offenseCardChoices.playerCards);
...
function pickOpponentDefenseCard({ state, action }) {
  ...
  // to this point
  // TODO alos pay attention to existing opponents cards on the ship or planet
  return {
    state: _.assign({}, state, {
      defenseCardChoices: _.assign({}, state.defenseCardChoices, {
...
function getStrength(cards) {
  return cards.reduce((curr, card) => ({
    ...
    offense: curr.offense + (card.offense || 0),
    defense: curr.defense + (card.defense || 0),
  }), {
    strength: 0,
    offense: 0,
    defense: 0,
...
function checkForStalemate(state) {
  ...
  const checkIfStalled = zone => zone.playerCards.length === 0 && zone.aiCards.length === 0;
  if (!activePlayerCards && !activeAiCards && checkIfStalled(state.planet) &&
      checkIfStalled(state.ships.playerShip) && checkIfStalled(state.ships.aiShip)) {
    return true;
  }
  return false;
}
...
function combatHelper(zone) {
  ...
  const { strength: plStrength,
    offense: plOffense,
    defense: plDefense } = getStrength(zone.playerCards);
  const { strength: aiStrength,
    offense: aiOffense,
    defense: aiDefense } = getStrength(zone.aiCards);
  const plDefenseBonus = plStrength > aiStrength ? 1 : 0;
  const aiDefenseBonus = plStrength < aiStrength ? 1 : 0;
  const plNewStrength = (plStrength + plDefense + plDefenseBonus)
    - (aiStrength + aiOffense);
  const aiNewStrength = (aiStrength + aiDefense + aiDefenseBonus)
    - (plStrength + plOffense);
