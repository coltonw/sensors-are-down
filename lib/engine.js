...
const pickRandomKeys = (obj, count = 1) => {
  const keys = Object.keys(obj);
  return _.take(_.shuffle(keys), count);
};

const pickRandomCards = (cardsObj, count) => {
  const cardIds = pickRandomKeys(cardsObj, count);
...
function presentOffenseChoices({ state, action }) {
  return {
    state: {
      offenseCardChoices: {
        playerCards: pickRandomCards(state.playerDeck, 2),
        aiCards: pickRandomCards(state.playerDeck, 2),
      },
    },
    action };
}
