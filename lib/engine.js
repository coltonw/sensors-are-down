...
type OtherState = {
  prevOffenseCardChoices: ?{
    playerCards: CardsObj,
    aiCards: CardsObj,
  },
  prevDefenseCardChoices: ?{
    playerCards: CardsObj,
    aiCards: CardsObj,
  },
...
type State = StateMachine & Zones & OtherState;
...
function pickOpponentOffenseCard({ state, action }: SubReducerObj): SubReducerObj {
  ...
  const stateWithPickedCards: any = { // using any here because of bugs with flow object spread :/
    ...state,
    offenseCardChoices: {
      ...state.offenseCardChoices,
...
const game = (state: State = gameInitialState, action) => {
  switch (action.type) {
    case START_GAME: {
      return _.flow(
        createDecks,
